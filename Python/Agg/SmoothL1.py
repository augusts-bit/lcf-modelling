...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=32, shuffle=True) # batch size 32 (from param test)

# Create a DataLoader for the test set
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=32, shuffle=False)

# Out of the tested options, the following combination of hyperparameters achieved the lowest loss:
# Optimizer: SGD, LR: 0.1, hidden size: 256, layers: 1, dropout: 0.0, epochs: 100, batch size: 32, weight init: xavier, activation: tanh
# Now with learning rate scheduler (if test loss plateaus, lr will be lowered)
...
loss_fn = nn.SmoothL1Loss() # SmoothL1Loss!
...
# Epochs
num_epochs = 100 # The higher, the longer it takes but the better idea of the model's fiting ability
...
for epoch in range(num_epochs):
  ...
  scheduler.step(np.mean(epoch_testloss))
...
pred_df.to_csv('LSTM_predict_SmoothL1.csv')
