...
## IIASA.TRAIN
linkIIASAtrain = "C:/Users/augus/Documents/Studie/MGI/Thesis/Rob's/Data/Processed/IIASAChangeFiltered.gpkg"
nameBands <- st_layers(linkIIASAtrain)
coords = read.csv("C:/Users/augus/Documents/Studie/MGI/Thesis/Rob's/Data/Processed/IIASAChangeCoords.csv")
coords = coords[,c("x", "y")]
...
change_train2015 <- getDataDF("2014|2015|2016")
change_train2016 <- getDataDF("2015|2016|2017")
change_train2017 <- getDataDF("2016|2017|2018")
change_train2018 <- getDataDF("2017|2018|2019")

# Column bind harmonics
change_train2015 <- cbind(change_train2015, HarmMetrics2015)
change_train2016 <- cbind(change_train2016, HarmMetrics2016)
change_train2017 <- cbind(change_train2017, HarmMetrics2017)
change_train2018 <- cbind(change_train2018, HarmMetrics2018)
...
change_train2015 <- merge(change_train2015,trainingRaw2015, by = c("sample_id"))
change_train2016 <- merge(change_train2016,trainingRaw2016, by = c("sample_id"))
change_train2017 <- merge(change_train2017,trainingRaw2017, by = c("sample_id"))
change_train2018 <- merge(change_train2018,trainingRaw2018, by = c("sample_id"))
...
colnames(change_train2015)[2] = "x"
colnames(change_train2015)[3] = "y"
colnames(change_train2016)[2] = "x"
colnames(change_train2016)[3] = "y"
colnames(change_train2017)[2] = "x"
colnames(change_train2017)[3] = "y"
colnames(change_train2018)[2] = "x"
colnames(change_train2018)[3] = "y"
...
similar_id_df <- subset(similar_id_df, sample_id %in% change_train2016$sample_id)
similar_id_df <- subset(similar_id_df, sample_id %in% change_train2017$sample_id)
similar_id_df <- subset(similar_id_df, sample_id %in% change_train2018$sample_id)

change_train2015 <- subset(change_train2015, sample_id %in% similar_id_df$sample_id)
change_train2016 <- subset(change_train2016, sample_id %in% similar_id_df$sample_id) 
change_train2017 <- subset(change_train2017, sample_id %in% similar_id_df$sample_id) 
change_train2018 <- subset(change_train2018, sample_id %in% similar_id_df$sample_id)
...
change_train <- change_train[with(change_train,order(sample_id,reference_year)),]

# Rearrange and drop some columns so it is clear to understand

# Old colnames
# [1] "sample_id"      "x"              "y"              "b1_median"      "b2_median"      "b3_median"      "b4_median"      "b5_median"     
...
change_train <- change_train[, c("sample_id", "location_id", "validation_id", "reference_year", "x", "y", "b1_median", "b2_median", "b3_median", 
...
write.csv(change_train, "C:/Users/augus/Documents/Studie/MGI/Thesis/DataAugust/LSTM/Input/train20152018.csv")
