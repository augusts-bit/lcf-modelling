...
for (date in acqdates){
  ...
  # Add band medians (LSTM input)
  # Bands 
  # Convert to numeric
  b1 = as.data.frame(sapply(b1_TS[,NewColDates], as.numeric))
  b2 = as.data.frame(sapply(b2_TS[,NewColDates], as.numeric))
  b3 = as.data.frame(sapply(b3_TS[,NewColDates], as.numeric))
  b4 = as.data.frame(sapply(b4_TS[,NewColDates], as.numeric))
  b5 = as.data.frame(sapply(b5_TS[,NewColDates], as.numeric))
  b6 = as.data.frame(sapply(b6_TS[,NewColDates], as.numeric))
  b7 = as.data.frame(sapply(b7_TS[,NewColDates], as.numeric))
  
  # Medians (and IQRs for VIs)
  b1_median = as.numeric(apply(b1, 1, function(x){median(x, na.rm = TRUE)}))
  b2_median = as.numeric(apply(b2, 1, function(x){median(x, na.rm = TRUE)}))
  b3_median = as.numeric(apply(b3, 1, function(x){median(x, na.rm = TRUE)}))
  b4_median = as.numeric(apply(b4, 1, function(x){median(x, na.rm = TRUE)}))
  b5_median = as.numeric(apply(b5, 1, function(x){median(x, na.rm = TRUE)}))
  b6_median = as.numeric(apply(b6, 1, function(x){median(x, na.rm = TRUE)}))
  b7_median = as.numeric(apply(b7, 1, function(x){median(x, na.rm = TRUE)}))
  
  # add
  valiDF$b1_median <- b1_median
  valiDF$b2_median <- b2_median
  valiDF$b3_median <- b3_median
  valiDF$b4_median <- b4_median
  valiDF$b5_median <- b5_median
  valiDF$b6_median <- b6_median
  valiDF$b7_median <- b7_median
  ...
  valiDF <- valiDF[!is.na(valiDF$b1_median),]
  valiDF <- valiDF[!is.na(valiDF$b2_median),]
  valiDF <- valiDF[!is.na(valiDF$b3_median),]
  valiDF <- valiDF[!is.na(valiDF$b4_median),]
  valiDF <- valiDF[!is.na(valiDF$b5_median),]
  valiDF <- valiDF[!is.na(valiDF$b6_median),]
  valiDF <- valiDF[!is.na(valiDF$b7_median),]
  ...
  write.csv(vali, paste0(paste0(linkData, "Processed/Validation/Dense_Window/witBands/vali"),acqdates[n],".csv"))
