...
SCM = function(preded, observed, agreement=min, disagreement="SCM", 
...
plot.scm = function(scm, ...)
{
  ...
  {
    P_plot = lattice::levelplot(t(scm$P), xlab="Observed", ylab="preded", sub="Centre values", ...)
    U_plot = lattice::levelplot(t(scm$U), xlab="Observed", ylab="preded", sub="Uncertainty", ...)
...
accuracy.scm = function(scm)
{
  ...
  P_UA_s = (diag(scm$P)*P_kp) / (P_kp^2 - U_kp^2) # Problems when we have uncertainty == predion, division by 0
